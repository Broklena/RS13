...
// ReconStrike V13.4 -- Layer 1 Core (with encryption support)
...
(function () {
  ...
  // ── SECURE (defined before storage because storage reads it) ──
  var secure = (function () {
    var key = null;
  ...
  var storage = (function () {
    ...
    async function put(store, record, key) {
      ...
      // Store in-memory the plaintext version always
      memGet(store).set(record.id, record);
    ...
    async function getAll(store) {
      ...
      if (!secure.enabled()) {
        // Not unlocked: return in-memory plaintext if available
        var fromMem = Array.from(memGet(store).values());
        if (fromMem.length) return fromMem;
        // Otherwise expose encrypted markers as-is
        return rows;
      }
  ...
  setTimeout(function () { storage.pruneAll().catch(function () {}); }, 3000);

  eventBus.emit('core:ready', { version: '13.4', stores: STORES.length, pool: worker.size });
