...
// ReconStrike V17 -- Final UI (14 tabs, orchestrator-integrated)
...
+ '<div class="rs-hd">'
+ '<div class="rs-brand"><div class="rs-logo">⚔</div><div class="rs-name">ReconStrike</div><div class="rs-ver">V17</div></div>'
...
function renderOperator(d){
  ...
  if(!op){ h += emptyHTML('وحدة OPERATOR غير محمّلة', '⏱'); return h; }
...
function renderDarwin(d){
  var dw = core.darwin;
  var h = '';
  if(!dw){ h += emptyHTML('وحدة DARWIN غير محمّلة', '⌬'); return h; }
  ...
  h += '<div class="rs-item" id="rs-dw-export" style="cursor:pointer"><div class="rs-item-h"><span class="rs-tag i">↓</span><div class="rs-item-t">تصدير النظام البيئي</div></div></div>';
  ...
  if(hall.length){
    hall.slice(0, 15).forEach(function(x, i){
      ...
      h += itemHTML('h' + i, (x.impact || 0).toFixed(1), 'p', String(x.sig || '').slice(0, 40), '', detail);
    });
  ...
  setTimeout(function(){
    ...
    wire('dw-stop', function(){ dw.stop(); toast('إيقاف'); render(); });
...
function renderSynapse(d){
  var sy = core.synapse;
  var h = '';
  if(!sy){ h += emptyHTML('وحدة SYNAPSE غير محمّلة', '◎'); return h; }
  ...
  h += '<div class="rs-item" id="rs-sy-export" style="cursor:pointer"><div class="rs-item-h"><span class="rs-tag i">↓</span><div class="rs-item-t">تصدير التعلّم</div></div></div>';
  ...
  setTimeout(function(){
    ...
    wire('sy-run', async function(){
      ...
      } else toast('فشل: ' + (r.reason || r.error || ''));
      render();
    });
    wire('sy-stop', function(){ sy.stop(); toast('إيقاف'); render(); });
...
function renderDash(d){
  ...
  if(ORCH){
    var orchSt = ORCH.status();
    h += '<div class="rs-sec">';
    h += '<div style="background:rgba(34,197,94,.06);border:1px solid rgba(34,197,94,.25);border-radius:8px;padding:8px 12px;color:#4ade80;font-size:11px">'
      + '⚙ المُنظِّم نشط -- Hooks: ' + Object.keys(orchSt.hooks).filter(function(k){ return orchSt.hooks[k].wrapped; }).length
      + ' · مشتركون: ' + Object.keys(orchSt.hooks).reduce(function(a, k){ return a + orchSt.hooks[k].subscribers; }, 0)
      + ' · طابور: ' + orchSt.scheduler.queued
      + '</div>';
    h += '</div>';
  }
...
function renderPulsar(d){
  ...
  if(ORCH){
    ...
    h += '<div class="rs-sec">';
    h += '<div class="rs-item" style="cursor:default">'
      + '<div class="rs-item-h"><span class="rs-tag n">⚙</span><div class="rs-item-t">معدل عالمي</div></div>'
      + '<div class="rs-item-s">رموز متاحة: ' + orchSt.rateLimiter.tokens + ' / 10 · منتظرون: ' + orchSt.rateLimiter.waiters + '</div>'
      + '</div>';
    h += '</div>';
  }
...
function renderGhost(d){
  ...
  if(ORCH){
    var orchSt = ORCH.status();
    h += '<div class="rs-sec"><div class="rs-sec-h"><div class="rs-sec-t">حالة المُنظِّم</div><div class="rs-sec-c">V' + orchSt.version + '</div></div>';
    h += '<div class="rs-item" style="cursor:default">'
      + '<div class="rs-item-h"><span class="rs-tag n">Hooks</span><div class="rs-item-t">مركّزة</div></div>'
      + '<div class="rs-item-s">'
      + 'fetch: ' + orchSt.hooks.fetch.subscribers + ' مشترك · '
      + 'xhr: ' + orchSt.hooks.xhr.subscribers + ' · '
      + 'ws: ' + orchSt.hooks.ws.subscribers + ' · '
      + 'beacon: ' + orchSt.hooks.beacon.subscribers + ' · '
      + 'sse: ' + orchSt.hooks.eventsource.subscribers
      + '</div>'
      + '</div>';
    h += '<div class="rs-item" style="cursor:default">'
      + '<div class="rs-item-h"><span class="rs-tag i">طوابير</span><div class="rs-item-t">Scheduler / Rate</div></div>'
      + '<div class="rs-item-s">scheduler: ' + orchSt.scheduler.queued + ' · rate tokens: ' + orchSt.rateLimiter.tokens + ' · cleanups: ' + orchSt.cleanups + '</div>'
      + '</div>';
    h += '</div>';
  }
  ...
  h += '<div class="rs-item" id="rs-g-orch-wipe" style="cursor:pointer"><div class="rs-item-h"><span class="rs-tag h">⚙</span><div class="rs-item-t">تفريغ المُنظِّم</div></div><div class="rs-item-s">context + signal bus + queues</div></div>';
  ...
  setTimeout(function(){
    ...
    wire('g-orch-wipe', function(){
      if(!window.confirm('تفريغ المُنظِّم (context, signals, queues)؟')) return;
      ghost.wipeOrchestrator();
      toast('تم');
      render();
    });
...
// High-frequency events → through orchestrator signal bus (debounced)
if(ORCH && typeof ORCH.subscribe === 'function'){
  [
    'finding:new', 'crawler:page',
    'pulsar:hit', 'pulsar:progress',
    'argus:tick', 'argus:hit',
    'synapse:progress', 'darwin:progress'
  ].forEach(function(ev){
    ORCH.subscribe(ev, function(payload){
      scheduleRender();
      // Specific UI hints
      if(ev === 'finding:new' && payload && payload.count){
        // silent -- avoid toast spam
      }
    });
  });
} else {
  // Fallback: direct subscription without debounce
  ['finding:new', 'crawler:page', 'pulsar:hit', 'pulsar:progress', 'argus:tick', 'argus:hit', 'synapse:progress', 'darwin:progress'].forEach(function(ev){
    eventBus.on(ev, scheduleRender);
  });
}

// Low-frequency events → direct subscription (immediate response)
...
updateLockIcon();
render().catch(function(){});
setTimeout(function(){ toast('ReconStrike V17 جاهز'); }, 500);
