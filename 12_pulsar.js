...
// PULSAR V2 -- expanded fuzzing + stealth engine
...
(function () {
  ...
  var Stealth = (function () {
    ...
    function nextDelay(base) {
      ...
      var jitter = base * (0.4 + g * 1.6);
      if (Math.random() < 0.04) jitter += 1500 + Math.random() * 3000;
    ...
    function observe(status) {
      ...
      if (recentStatuses.length > 20) recentStatuses.shift();
      var rate = recentStatuses.filter(function (s) {
        return s === 429 || s === 503;
      }).length;
      if (rate >= 5) {
        backoffUntil = Date.now() + 15000 + Math.random() * 15000;
      } else if (rate >= 3) {
        backoffUntil = Date.now() + 5000 + Math.random() * 5000;
      }
    }
    ...
    async function wait() {
      var now = Date.now();
      if (now < backoffUntil) {
        await new Promise(function (r) { setTimeout(r, backoffUntil - now); });
      }
      await new Promise(function (r) { setTimeout(r, nextDelay()); });
    }

    function status() {
      return {
        backoff: Math.max(0, backoffUntil - Date.now()),
  ...
  var PARAMS = [].concat(
    ...
    ['query','mutation','subscription','operationName','variables','extensions','persistedQuery','sha256Hash','__typename','__schema','__type'],
  ...
  async function computeBaseline() {
    ...
    try {
      var r = await fetch(rand, {
        credentials: 'include',
        redirect: 'manual',
        headers: Stealth.buildHeaders('navigation')
      });
      Stealth.absorbCookies(r);
      Stealth.observe(r.status);
      var t = await r.text();
      baseline = { status: r.status, len: t.length, ct: r.headers.get('content-type') || '' };
    } catch (e) {
